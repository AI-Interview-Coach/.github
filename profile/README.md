...
### GPT 기반 질문 생성 시 중복 및 형식 오류 문제

#### 문제 상황 1: 질문 중복 생성

**초기 구현 방식**
- 사용자가 요청한 질문 개수만큼 GPT API를 반복 호출
- 예: 3개 질문 필요 시 → GPT API를 3번 순차적으로 호출

```python
# 초기 방식 (문제 있는 코드)
questions = []
for i in range(count):  # count = 3
    question = get_completion(prompt)  # API 1회 호출
    questions.append(question)
```

**발생한 문제**
1. **중복 질문 생성**: GPT가 이전에 생성한 질문을 기억하지 못해 동일하거나 유사한 질문 반복 생성
2. **시간 소요**: API 호출마다 약 2-3초 소요 → 3개 질문 생성에 6-9초 필요
3. **비효율적 API 사용**: 불필요하게 많은 API 호출로 비용 증가

**예시**
```
질문 1: "데이터베이스 정규화의 목적을 설명해주세요."
질문 2: "데이터베이스 정규화가 왜 필요한지 말씀해주세요." ← 중복!
질문 3: "자료구조에서 스택과 큐의 차이를 설명해주세요."
```

---

#### 해결 방법 1: 일괄 생성 후 랜덤 선택

**개선된 방식**
- GPT API를 **1회만 호출**하여 **10개의 질문을 한 번에 생성**
- 생성된 10개 중에서 사용자가 요청한 개수만큼 랜덤 선택

```python
# 개선된 코드
def first_question(count):
    cq_count = 10  # 항상 10개 질문 생성
    
    # 1. GPT에게 10개 질문 한 번에 요청
    prompt = f'''
    유능한 IT 기업의 기술 면접관으로서, 다양한 컴퓨터 공학 키워드를 바탕으로 
    심도 있는 기술 면접 질문을 {cq_count}개 생성해주세요.
    
    [질문 생성 조건]
    - 같은 키워드의 질문이나 유사한 질문은 피해야 합니다.
    - 무조건 다양한 질문을 만들어주세요.
    '''
    
    # 2. API 1회 호출로 10개 질문 생성
    result = get_completion(prompt)
    resultArr = result.split("@@@")
    
    # 3. 10개 중 count개만큼 랜덤 선택
...
**개선 효과**
- ✅ **중복 제거**: 10개를 한 번에 생성하면서 "유사한 질문 피하기" 조건 명시
- ✅ **시간 단축**: API 호출 1회로 단축 (6-9초 → 2-3초)
- ✅ **비용 절감**: API 호출 횟수 감소 (3회 → 1회)
- ✅ **다양성 확보**: 10개 중 선택하므로 매번 다른 조합 가능
...
**발생한 문제**
- 배열 파싱 실패 → `split("@@@")` 후 개수가 10개가 아님
- 번호 형식 불일치 → `1.`이 아닌 `1)` 또는 번호 없음
- 질문 추출 불가 → 프로그램 오류 발생
...
생성된 질문이 올바른 형식인지 검사하고, 형식이 맞지 않으면 재생성을 반복하는 로직 추가:
...
def first_question(count):
    ...
    while checkFlag:
        ...
        # 2. 질문 개수 검증
        if len(resultArr) != 10:
            continue  # 10개가 아니면 재생성
        
        # 3. 번호 형식 검증
        numbercheck = True
        for i in range(cq_count):
            resultArr[i] = resultArr[i].strip()  # 공백 제거
...
| 검증 단계 | 조건 | 실패 시 동작 |
|----------|------|------------|
| **1단계: 개수 검증** | `len(resultArr) == 10` | 재생성 요청 |
| **2단계: 번호 형식 검증** | `"1.", "2.", ..., "10."` 형식 확인 | 재생성 요청 |
| **3단계: 번호 제거** | 순수 질문 텍스트만 추출 | - |
| **4단계: 최종 확인** | 모든 검증 통과 | 루프 종료 |

**개선 효과**
- ✅ **안정성 확보**: 형식 오류로 인한 프로그램 중단 방지
- ✅ **자동 복구**: 형식이 맞지 않으면 자동으로 재생성
- ✅ **일관된 데이터**: 항상 "1.", "2.", ... 형식의 10개 질문 보장
- ✅ **사용자 경험 개선**: 형식 오류 없이 안정적인 질문 제공
...
↓
GPT 호출 1회 → 질문 1 생성
GPT 호출 2회 → 질문 2 생성 (중복 가능)
GPT 호출 3회 → 질문 3 생성 (중복 가능)
↓
형식 오류 발생 시 프로그램 중단
```
...
| 문제 | 해결책 | 효과 |
|------|--------|------|
| 질문 중복 | 10개 일괄 생성 후 선택 | 다양성 확보, 시간 단축 |
| API 호출 과다 | 1회 호출로 통합 | 비용 절감, 속도 향상 |
| 형식 불일치 | 검증 루프 추가 | 안정성 확보, 자동 복구 |
